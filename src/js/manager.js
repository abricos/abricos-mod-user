...
Component.entryPoint = function(NS){
    ...
    YAHOO.extend(UsersWidget, Brick.widget.TablePage, {
        ...
        showUserEditor: function(userid){
            ...
            new NS.UserEditorPanel(userid || 0, function(){
                __self.refresh();
            });
        },
    ...
    YAHOO.extend(UserEditorPanel, Brick.widget.Dialog, {
        ...
        elv: function(name){
            return getValue(this.el(name));
        },
        setelv: function(name, value){
            setValue(this.el(name), value);
        },
        initTemplate: function(){
        ...
        onClick: function(el){
            if (this.userGroupWidget.onClick(el)){
                return true;
            }
        ...
        showPassField: function(){
            this.el('passbutton').style.display = 'none';
    ...
    YAHOO.extend(GroupEditorPanel, Brick.widget.Dialog, {
        el: function(name){
            return Dom.get(this._TId['geditor'][name]);
        },
        elv: function(name){
            return getValue(this.el(name));
        },
    ...
    RolesWidget.prototype = {
        ...
        render: function(){
            ...
            DATA.get('modactionlist').getRows().foreach(function(row){
                modules[row.cell['md']] = true;
            });

            var lst = "", TM = this._TM;

            for (var nn in modules){
                if (!Brick.componentExists(nn, 'cp')){
                    continue;
                }
